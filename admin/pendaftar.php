...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'verify' && !empty($pendaftar_id)) {
        $result = $db->update('pendaftar', ['status' => 'confirmed'], 'id = ?', [$pendaftar_id]);
        ...
        $result = $db->update('pendaftar', ['status' => 'rejected'], 'id = ?', [$pendaftar_id]);
...
$kategori_list = $db->fetchAll("SELECT id, nama FROM kategori_lomba ORDER BY nama");

// Get statistics
$total_pendaftar = $db->fetch("SELECT COUNT(*) as total FROM pendaftar")['total'];
$pending_pendaftar = $db->fetch("SELECT COUNT(*) as total FROM pendaftar WHERE status = 'pending'")['total'];
$confirmed_pendaftar = $db->fetch("SELECT COUNT(*) as total FROM pendaftar WHERE status = 'confirmed'")['total'];
$rejected_pendaftar = $db->fetch("SELECT COUNT(*) as total FROM pendaftar WHERE status = 'rejected'")['total'];
?>
