...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        if ($action === 'create') {
            ...
            $res = $db->update('b_workshop', $data, 'id = ?', [$id]);
        ...
        $id = intval($_POST['id'] ?? 0);
        $deleted = $db->delete('b_workshop', 'id = ?', [$id]);
...
// Fetch list
$list = $db->fetchAll('SELECT * FROM b_workshop ORDER BY created_at DESC');
?>
<!DOCTYPE html>
<html lang="id">
<head>
