...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'approve') {
        $upd = $db->update('b_workshop_pendaftar', ['status' => 'approved'], 'id = ?', [$id]);
        $message = $upd ? 'Pendaftar disetujui' : 'Gagal menyetujui';
        $message_type = $upd ? 'success' : 'danger';
    } elseif ($_POST['action'] === 'reject') {
        $upd = $db->update('b_workshop_pendaftar', ['status' => 'rejected'], 'id = ?', [$id]);
...
// Fetch
$pendaftar = $db->fetchAll("SELECT wp.*, w.judul FROM b_workshop_pendaftar wp JOIN b_workshop w ON wp.workshop_id = w.id $where_clause ORDER BY wp.created_at DESC", $params);
$workshops = $db->fetchAll('SELECT id, judul FROM b_workshop ORDER BY judul ASC');
?>
<!DOCTYPE html>
<html lang="id">
<head>
