...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_lomba') {
        ...
        // Update data lomba
        $db->query(
            "UPDATE kategori_lomba SET nama = ?, deskripsi = ?, periode_pendaftaran = ?, peserta = ?, durasi = ?, tempat = ?, status = ? WHERE id = ?",
            [$nama, $deskripsi, $periode_pendaftaran, $peserta, $durasi, $tempat, $status, $lomba_id]
        );
...
// Ambil semua lomba
$lomba_list = $db->fetchAll("SELECT * FROM kategori_lomba ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
