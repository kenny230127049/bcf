    ...
    if (isset($_FILES['bukti_transfer']) && is_array($_FILES['bukti_transfer']) && $_FILES['bukti_transfer']['error'] === 0) {
        ...
        if (in_array($file['type'], $allowed) && $file['size'] <= $max) {
            if (!is_dir('uploads/bukti_transfer')) {
                @mkdir('uploads/bukti_transfer', 0777, true);
            }
                ...
                <div class="card-body">
                    ...
                    <form method="post" enctype="multipart/form-data" id="wsPayForm">
                        <div class="text-center mb-3">
                            ...
                            <?php
                            // Tampilkan QRIS jika tersedia: coba beberapa nama file umum
                            $qrisCandidates = [
                                'images/pembayaran_qris.jpeg',
                                'images/pembayaran_qris.jpg',
                                'images/pembayaran_qris.png',
                                'images/qris.jpeg',
                                'images/qris.jpg',
                                'images/qris.png',
                            ];
                            $qrisSrc = '';
                            foreach ($qrisCandidates as $candidate) {
                            ...
                            if ($qrisSrc): ?>
                                <img src="<?= htmlspecialchars($qrisSrc) ?>" alt="QRIS" style="max-width: 280px; width: 100%; border-radius: 12px; box-shadow: 0 10px 20px rgba(0,0,0,0.1);">
                            <?php else: ?>
                                <div class="alert alert-warning">
                                    Gambar QRIS tidak ditemukan. Pastikan file berada di folder <code>images/</code> dengan nama salah satu dari:
                                    <code>pembayaran_qris.(jpeg|jpg|png)</code> atau <code>qris.(jpeg|jpg|png)</code>.
                                </div>
                            <?php endif; ?>
                        </div>
                        <div class="mb-3">
