    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($metode_pembayaran)) {
            ...
            // Validasi bukti transfer WAJIB ada dan valid
            $hasValidProof = false;
            $uploadedFilename = '';

            if (isset($_FILES['bukti_transfer']) && is_array($_FILES['bukti_transfer']) && ($_FILES['bukti_transfer']['error'] ?? UPLOAD_ERR_NO_FILE) == UPLOAD_ERR_OK) {
                $file = $_FILES['bukti_transfer'];
                $allowed_types = ['image/jpeg', 'image/jpg', 'image/png'];
                $max_size = 5 * 1024 * 1024; // 5MB

                if (in_array($file['type'], $allowed_types) && $file['size'] <= $max_size) {
                    $filename = 'bukti_transfer_' . time() . '_' . preg_replace('/[^A-Za-z0-9_\.-]/', '_', $file['name']);
            ...
            if (!$hasValidProof) {
                $message = 'Unggah bukti pembayaran (JPG/PNG, maks 5MB) sebelum melanjutkan.';
            } else {
                $payment_id = 'PAY' . date('YmdHis') . rand(100, 999);


                // Insert payment record HANYA jika bukti valid
                $payment_data = [
                    'id' => $payment_id,
            ...
            <div class="payment-container animate__animated animate__fadeInUp">
                ...
                <div class="summary-card">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-md-end">
                            ...
                            <h4 class="mb-0"><strong>Total: Rp <?= number_format($pendaftar['biaya'], 0, ',', '.') ?></strong></h4>
                        </div>
                    </div>
                </div>
                ...
                <form action="" method="post" id="paymentForm" enctype="multipart/form-data">
                    <div class="d-flex flex-column align-items-center">
                        ...
                        <img src="images/pembayaran_qris.jpg" alt="QRIS" class="qris-img">
