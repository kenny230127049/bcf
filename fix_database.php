...
// Drop alamat from b_users if exists
try {
    echo "<p>Menghapus kolom alamat dari tabel b_users...</p>";
    $db->query("ALTER TABLE b_users DROP COLUMN alamat");
    echo "<p style='color: green;'>✓ Kolom alamat di b_users berhasil dihapus</p>";
} catch (Exception $e) {
    echo "<p style='color: orange;'>⚠ Kolom alamat di b_users sudah tidak ada atau error: " . $e->getMessage() . "</p>";
}

// Tambah kolom timeline per lomba jika belum ada
try {
    echo "<p>Menambahkan kolom timeline_pendaftaran, timeline_seleksi, timeline_pengumuman ke b_kategori_lomba...</p>";
    $db->query("ALTER TABLE b_kategori_lomba ADD COLUMN timeline_pendaftaran VARCHAR(255) NULL AFTER deskripsi");
    $db->query("ALTER TABLE b_kategori_lomba ADD COLUMN timeline_seleksi VARCHAR(255) NULL AFTER timeline_pendaftaran");
    $db->query("ALTER TABLE b_kategori_lomba ADD COLUMN timeline_pengumuman VARCHAR(255) NULL AFTER timeline_seleksi");
    echo "<p style='color: green;'>✓ Kolom timeline berhasil ditambahkan</p>";
} catch (Exception $e) {
    echo "<p style='color: orange;'>⚠ Kolom timeline mungkin sudah ada atau error: " . $e->getMessage() . "</p>";
}

// Hapus kolom periode_pendaftaran jika ada
try {
    echo "<p>Menghapus kolom periode_pendaftaran dari b_kategori_lomba...</p>";
    $db->query("ALTER TABLE b_kategori_lomba DROP COLUMN periode_pendaftaran");
    echo "<p style='color: green;'>✓ Kolom periode_pendaftaran berhasil dihapus</p>";
} catch (Exception $e) {
    echo "<p style='color: orange;'>⚠ Kolom periode_pendaftaran sudah tidak ada atau error: " . $e->getMessage() . "</p>";
}

// Tambah kolom banner_path untuk gambar lomba jika belum ada
try {
    echo "<p>Menambahkan kolom banner_path ke b_kategori_lomba...</p>";
    $db->query("ALTER TABLE b_kategori_lomba ADD COLUMN banner_path VARCHAR(255) NULL AFTER icon");
    echo "<p style='color: green;'>✓ Kolom banner_path berhasil ditambahkan</p>";
} catch (Exception $e) {
    echo "<p style='color: orange;'>⚠ Kolom banner_path mungkin sudah ada atau error: " . $e->getMessage() . "</p>";
}
?>
