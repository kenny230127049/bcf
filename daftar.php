...
$kategori = $db->fetch("SELECT * FROM kategori_lomba WHERE id = ? AND status = 'aktif'", [$kategori_id]);
if (!$kategori) {
    header('Location: index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama) || empty($email) || empty($telepon) || empty($sekolah) || empty($kelas) || empty($alamat)) {
        ...
        if ($result) {
            ...
            $db->insert('user_pendaftaran', $userPendaftaranData);
            ...
            if ($jenis_lomba == 'kelompok') {
                ...
                for ($i = 1; $i <= $jumlah_anggota; $i++) {
                    ...
                    if (!empty($anggota_nama) && !empty($anggota_sekolah) && !empty($anggota_kelas)) {
                        ...
                        $db->insert('anggota_kelompok', $anggota_data);
                    }
                }
            }
